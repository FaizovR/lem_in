...
/*   Updated: 2020/09/12 10:40:19 by nalysann         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		is_room_line(char *line, t_room_type type)
{
	char		**words;
	long long	number;
	char		*endptr;
	...
	{
		if (type == R_SOURCE)
			ft_throw(NO_ROOM_S_MSG, E_INPUT);
		if (type == R_SINK)
			ft_throw(NO_ROOM_E_MSG, E_INPUT);
		free_split(words, 0);
		return (0);
	}
...
{
	if (ft_strequ(line, C_START) && *type != R_SOURCE)
	{
		if (*type == R_SINK)
			ft_throw(ALREADY_MARKED_MSG, E_INPUT);
		if (info->s != NO_ROOM)
			ft_throw(MULT_START_MSG, E_INPUT);
		info->s = EXST_ROOM;
		*type = R_SOURCE;
	}
	else if (ft_strequ(line, C_END) && *type != R_SINK)
	{
		if (*type == R_SOURCE)
			ft_throw(ALREADY_MARKED_MSG, E_INPUT);
		if (info->t != NO_ROOM)
