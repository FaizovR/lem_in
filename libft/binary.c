...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/08 19:24:17 by gjenkins          #+#    #+#             */
...
unsigned long long	ft_caste_binary(va_list args_list, t_flags *flags)
{
	if (flags->mod.hh)
...
static void			left_aligned_binary(t_flags *flags, int *counter, \
					char *str)
{
	int i;
...
static void			right_aligned_binary(t_flags *flags, int *counter,\
					char *result_str)
{
...
static void			ft_binary_help(t_flags *flags)
{
	if (flags->flag_hash)
	{
...
int					ft_print_binary(va_list args_list, t_flags *flags)
{
	...
	counter = 0;
	value = ft_caste_binary(args_list, flags);
	result_str = ft_ltoa_base(value, 2, 0);
	flags->arg_length = ft_strlen(result_str);
	if (flags->flag_precision && !flags->precision && value == 0)
		flags->arg_length = 0;
	if (!value && !flags->flag_precision)
		flags->flag_hash = 0;
	ft_binary_help(flags);
	if (flags->flag_precision)
		flags->flag_zero = 0;
	if (flags->flag_minus)
		left_aligned_binary(flags, &counter, result_str);
	else
		right_aligned_binary(flags, &counter, result_str);
